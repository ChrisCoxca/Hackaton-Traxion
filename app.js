...
async function handleGenerate() {
  ...
  try {
    ...
    renderKPIs(kpis, STATE.routeDistance);
...
function deployFleet(count, routePath) {
  ...
  for (let i = 0; i < count; i++) {
    /* Posición aleatoria a lo largo del path con pequeño spread */
    ...
    const lat = basePos.lat() + (Math.random() - 0.5) * spread;
    const lng = basePos.lng() + (Math.random() - 0.5) * spread;
    const pos = { lat, lng };
    ...
    const iw = new google.maps.InfoWindow({
      content: `<div style="font-family:'Space Grotesk',sans-serif;min-width:160px;padding:2px 0;">
        ...
        <div style="font-size:10px;color:#94a3b8;">Estado: <strong style="color:${color};">${status}</strong></div>
    ...
    /* Animación: mover el marcador ligeramente cada 3s */
    const animInterval = setInterval(() => {
      if (!STATE.fleetMarkers.find(m => m.marker === marker)) {
      ...
      const jitter = 0.0003;
      marker.setPosition({
        lat: curPos.lat() + (Math.random() - 0.5) * jitter,
        lng: curPos.lng() + (Math.random() - 0.5) * jitter,
      });
      /* Actualizar coordenadas en fleet panel */
      const coordEl = document.getElementById(`coords-${unitId}`);
      if (coordEl) {
        const p = marker.getPosition();
        coordEl.textContent = `${p.lat().toFixed(4)}, ${p.lng().toFixed(4)}`;
      }
    }, 3000);
    ...
    unitEl.innerHTML = `
      ...
      <div class="unit-status ${status === 'EN RUTA' ? 'en-ruta' : 'en-espera'}">${status}</div>
...
function renderKPIs(kpis, distKm) {
  /* Fuel KPI */
